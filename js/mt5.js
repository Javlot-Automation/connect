...
// Initialize MT5 form listeners
export function mt5Init() {
    const f = mt5GetFields();

    // Add input listeners to update button state on change
    const inputs = [f.email, f.accountNumber, f.accountPassword, f.server];
    inputs.forEach(input => {
        if (input) {
            input.addEventListener('input', mt5UpdateSubmitState);
        }
    });

    // Add checkbox listeners
    if (f.confirmCorrect) {
        f.confirmCorrect.addEventListener('change', mt5UpdateSubmitState);
    }
    if (f.confirmSecure) {
        f.confirmSecure.addEventListener('change', mt5UpdateSubmitState);
    }
...
export async function mt5Submit() {
    ...
    try {
        ...
        const response = await fetch('https://prod-webflow-form-alb-2082518894.us-west-2.elb.amazonaws.com/', {
            method: 'POST',
...
function showSuccessScreen() {
    const stepContent = document.querySelector('.step-content[data-step="3"] .step-content-body');
    ...
    const buttonGroup = document.querySelector('.step-content[data-step="3"] .button-group');
